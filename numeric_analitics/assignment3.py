...
class Assignment3:
    ...
    def integrate(self, f: callable, a: float, b: float, n: int) -> np.float32:
        ...
        if n==1:
            return np.float32((b-a) * f((a+b)/2))
        if n==2:
            return np.float32(0.5 * (b-a) * (f(a)+f(b)))
        use_t_case = False
        if a>1e-7 and ((b/a)>20):
            use_t_case = True

        if use_t_case:
            t = np.linspace(0, 1, n, dtype=np.float32)
            x_points_np = a + (b - a) * (t ** 4)
            y_points_np = f(x_points_np).astype(np.float32)
            ans_trapz = np.trapezoid(y_points_np,x_points_np)
            return np.float32(ans_trapz)

        if n%2 ==0:
            n -= 1

        x_points_np = np.linspace(a, b, n, dtype=np.float32)

        try: ################## use for debugging
            y_points_np = np.array(f(x_points_np), dtype=np.float32)
            if y_points_np.shape != x_points_np.shape:
                raise ValueError("np arrays don't match X and Y")
        except Exception:
            try:
                y_points_np = np.array([f(x) for x in x_points_np], dtype=np.float32)
            except:
                raise Exception("problem with assigning y values")


        h = np.float32((b-a) / (n-1))
        #for odd n vlaue we use the simpson 1/3 rule
        t_sum = y_points_np[0] + y_points_np[-1]
        t_sum += 4.0 * np.sum(y_points_np[1:-1:2])
        t_sum += 2.0 * np.sum(y_points_np[2:-1:2])
        return np.float32(t_sum * h / 3)
    ...
    def areabetween(self, f1: callable, f2: callable) -> np.float32:
        ...
        """
        ass2 = Assignment2()
        intersections_iter = ass2.intersections(f1,f2, 1,100)
        intersections = sorted(set(intersections_iter))
        if len(intersections) < 2:
            return np.float32(np.nan)

        total_area = np.float32(0.0)
        def functions_diff(x):
            return f1(x) - f2(x)

        for i in range (len(intersections)-1):
            curr = intersections[i]
            nxt = intersections[i+1]

            if nxt - curr < 1e-6:
                continue
            cur_section_area = self.integrate(functions_diff, curr, nxt, 50)
            total_area += np.abs(cur_section_area)
