...
def flip_to_bottom (pile,index):
    flip(pile,find_largest_pancake(pile,index))
    flip(pile,len(pile)-index)


def pancake_sort(pile):
    organized_pile = pile
    for i in range (len(organized_pile),1,-1):
        flip_to_bottom(organized_pile, i)
